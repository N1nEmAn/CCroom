...
export function GlobalBugsOverlay() {
  ...
  useEffect(() => {
    ...
    const getVisibleLogoAnchorCenter = (): { x: number; y: number } | null => {
      const anchors = Array.from(document.querySelectorAll<HTMLElement>("[data-ccroom-logo-anchor='true']"));
    ...
    const onLogoDragStop = () => {
      ...
      window.dispatchEvent(new CustomEvent("ccroom-logo-carry-progress", {
    ...
    window.addEventListener("ccroom-bugs-config-change", onConfigChanged as EventListener);
    window.addEventListener("ccroom-logo-drag-start", onLogoDragStart as EventListener);
    window.addEventListener("ccroom-logo-drag-stop", onLogoDragStop as EventListener);
    ...
    return () => {
      ...
      window.removeEventListener("ccroom-bugs-config-change", onConfigChanged as EventListener);
      window.removeEventListener("ccroom-logo-drag-start", onLogoDragStart as EventListener);
      window.removeEventListener("ccroom-logo-drag-stop", onLogoDragStop as EventListener);
